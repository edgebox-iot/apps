...
# Make sure directory where this runs is the directory where the script is located
cd "$(dirname "$0")"
cd ../.templates/
...
# Replace all instances of exactly "hello" (case sensitive) with $app_slug in all files
find . -type f -exec sed -i "s/$template_app_slug/$app_slug/g" {} +

# Replace all instances of exactly "Hello World" (case sentitive) with $app_name in all files
find . -type f -exec sed -i "s/$template_app_name/$app_name/g" {} +

# Replace all instances of excatly "EdgeApp Template" (case sensitive) with $app_description in all files
find . -type f -exec sed -i "s/$template_app_description/$app_description/g" {} +
